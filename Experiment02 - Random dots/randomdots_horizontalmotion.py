...
lib_path = os.path.abspath(os.path.join('..','_Libraries')) 									# add libraries folder
sys.path.append(lib_path) 																		# to python system path

from rlabs_libutils import * 																	# utilities library
from rlabs_libtobii import EyetrackerBrowser, MyTobiiController             					# this is OUR library for the tobii eyetracker

from pyglet.gl import glClearColor																# for the background color
from pyglet import clock 																		# to show fps
import time 																					# to track time...
import numpy as np 																				# import numpy to deal with arrays as in matlab
import ConfigParser                                                         					# read parameter files
from collections import OrderedDict 															# to sort parameters
...
def main(
	...
	tp = OrderedDict([(k, map(float,v.split(',')) if ',' in v else float(v)) \
		for k,v in tp.items('asection')])         												# read parameters of config_file in OrderedDict
	...
	for trial_counter in range(numtrials):                                  					# for each trial 
		...
		trial 	 = trial_idx[trial_counter] 													# get trial number
		if numtrials > 1:
