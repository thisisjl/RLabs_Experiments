...
class MyCalibration:
    ...
    def run(self,tracker, on_calib_done):
        ...
        ######################################################
        ## SHOW INSTRUCTIONS FOR CALIBRATION
        ######################################################
        ...
        # self.tracker.StartCalibration(lambda error, r: glib_idle_add(self.on_calib_start, error, r))
        self.tracker.StartCalibration(lambda error, r: self.on_calib_start(error, r))
        ...
        for point in points:                                                    # for each point
            ...
            ######################################################  
            ## Add point to eyetracker (not scaled)
            ######################################################
            p.x, p.y = point

            self.add_point_completed = False
            # self.tracker.AddCalibrationPoint(p, lambda error, r: glib_idle_add(self.on_add_completed, error, r))
            self.tracker.AddCalibrationPoint(p, lambda error, r: self.on_add_completed(error, r))
            
            while not self.add_point_completed:
                pass

            # self.add_point(point)                                                # add point to tracker calibration
            self.point_index += 1 
        ...
        self.tracker.ComputeCalibration(lambda error, r: self.on_calib_compute(error, r, MyWin))
        # tracker.ComputeCalibration(lambda error, r: glib_idle_add(self.on_calib_compute, error, r))
        ...
        # self.tracker.ComputeCalibration(lambda error, r: self.on_calib_compute(error, r))                                                           
        # MyWin.close()
    ...
    def on_calib_start(self, error, r):
        # print 'calibration - on_calib_start'
        if error:
            self.on_calib_done(False, "Could not start calibration because of error. (0x%0x)" % error)
            return False
    ...
    def wait_for_add(self):
        # print 'calibration - wait_for_add'
        self.point_index += 1
        self.redraw()
    ...
    def on_add_completed(self, error, r):
        # print 'MyCalibration - on_add_completed'
        if error:
            self.on_calib_done(False, "Add Calibration Point failed because of error. (0x%0x)" % error)
            return False
    ...
    def on_calib_compute(self, error, r, MyWin):
        print 'calibration - on_calib_compute'
        # if error == 0x20000502:
        #     print "CalibCompute failed because not enough data was collected"
        #     # MyWin.close()
        #     self.on_calib_done(False, "Not enough data was collected during calibration procedure.")
...
class MyTobiiController:
    ...
    def write_eyetracker_data_file2(self):
        """
        Uses the complete GazeData array from tobii.controller.
        Based on the libtobii.TobiiController's flushData() method
        """
            ...
            for g, e in itertools.izip_longest(self.gazeData,self.input_events):
                ...
                if e is not None:
                    ...
                    if e.name == 'InputEvent':
                        ...
                        code = 8 if 'START' in e.type else -8
                        e.id = (e.ETtime - timeStampStart)/1000.0
                        
                    # compute ET time of event in ms
